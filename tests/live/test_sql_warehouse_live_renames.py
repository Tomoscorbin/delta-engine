...
def test_sync_replaces_a_composite_foreign_key_when_one_local_column_is_renamed(
    ...
    before_foreign_keys = read_live_table(live_connection, child_name)["foreign_keys"]
    assert len(before_foreign_keys) == 2
    before_name = before_foreign_keys[0][0]
    assert isinstance(before_name, str) and before_name
    assert before_foreign_keys == (
        (before_name, "account_num", parent_name, "account_id"),
    ...
    assert isinstance(after_name, str) and after_name
    assert after_foreign_keys == (
        (after_name, "account_id", parent_name, "account_id"),
        (after_name, "tenant_id", parent_name, "tenant_id"),
    )
    assert engine.sync(renamed_child, parent).has_changes is False
