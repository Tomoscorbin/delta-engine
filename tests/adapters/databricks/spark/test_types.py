...
def test_rendered_types_parse_in_the_spark_sql_parser(spark, case) -> None:
    ...
    frame = spark.sql(f"SELECT CAST(NULL AS {render_data_type(data_type)}) AS value")
...
def test_rendered_types_round_trip_through_describe_as_json(spark, case) -> None:
    ...
    spark.sql(f"CREATE TABLE {table_name} (value {render_data_type(data_type)}) USING parquet")
    try:
        [row] = spark.sql(f"DESCRIBE EXTENDED {table_name} AS JSON").collect()
        document = json.loads(row[0])
        assert data_type_from_json(document["columns"][0]["type"]) == data_type
    finally:
