...
| `UndeclaredColumnRename`                | A diff both drops a column and adds one of the same or a widenable type — a rename edited straight into the declaration would execute as silent data loss | Split the unrelated drop and add across two syncs                                                      |
