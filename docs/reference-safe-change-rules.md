...
| Property value format               | A value the catalog would reject: booleans must be lowercase `true`/`false`, retention durations `interval <n> <unit>`, `delta.dataSkippingNumIndexedCols` an integer >= -1, `delta.columnMapping.mode` `none`/`name` |
| Column names needing column mapping | Special characters (spaces, `,;{}()=`, tabs, newlines) without `delta.columnMapping.mode='name'` declared                                                                                                             |
