...
| `UndeclaredColumnRename`                | A diff both drops a column and adds one of the same or a widenable type — a rename edited straight into the declaration would execute as silent data loss | Declare the rename with `renamed_from`, or split an unrelated drop and add across two syncs            |
