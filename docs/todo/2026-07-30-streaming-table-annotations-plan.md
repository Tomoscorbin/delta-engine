...
**Status:** implemented 2026-07-31 — every task landed, the unchecked boxes below notwithstanding. One deliberate deviation from the Architecture note: `application/scopes.py` keeps four parallel literal aspect sets rather than composing `METADATA_ASPECTS` from `ANNOTATION_ASPECTS | KEY_ASPECTS`, so each scope states its own definition instead of deriving it from a neighbour. The containment `tags ⊂ annotations ⊂ metadata ⊂ full` is therefore a checked property, asserted by `test_the_scopes_form_a_containment_lattice`, rather than a structural one — and `COMMENT_ASPECTS` and `KEY_ASPECTS` were never introduced, having no second caller.
