...
**Status:** Consolidated review; completed and partial findings are marked
below, and the remaining findings await agreement and implementation
...
corrections and correctness-review status refresh from
[#327](https://github.com/Tomoscorbin/delta-engine/pull/327) accompany this
...
`✅` means the finding is resolved at the scope described here. `◐` means
later work removed part of the problem while a narrower remainder stays open.
...
[PR #335](https://github.com/Tomoscorbin/delta-engine/pull/335) replaced the
public string-to-`frozenset[TableAspect]` lowering with a closed, ordered
`TableScope` value. The minimum scope for each aspect now has one typed owner,
and callers ask `scope.manages(aspect)` instead of comparing raw sets.

The three reconciliation meanings remain. Unmanaged properties are omitted
from diffing, unmanaged foreign keys are excluded from relationship edges, and
most other unmanaged drift is computed then rejected. The proposed explicit
per-aspect mode is therefore narrower now, but the finding is not complete.
...
`TableRun` expose one derived `TableChangeState`, so text rendering no longer
reconstructs whether work was planned, applied, blocked, or partially applied.
[PR #328](https://github.com/Tomoscorbin/delta-engine/pull/328) then made
...
- [x] [PR #343](https://github.com/Tomoscorbin/delta-engine/pull/343) removed
  ...
  identity.
- [x] [PR #344](https://github.com/Tomoscorbin/delta-engine/pull/344) made the
  public and domain constraint constructors establish their intrinsic input
  invariants.
...
rendering opportunities remain open.

## 17. Hide property-dependent capabilities behind property policy

### Cause

`PropertyPolicy` owns managed keys, accepted values, observed projection, and
permitted transitions. Several callers still interpret the raw declared values
themselves:

- rename and column-name admission compare column mapping mode with `"name"`;
- change-data-feed naming compares its property with `"true"`;
- type-change validation compares type widening with `"true"`; and
- column-drop validation interprets column mapping again.

The callers should continue to own what each capability permits in their
context, but they should not each know the backend spelling that activates it.

### Recommended direction

Let `PropertyPolicy` derive a small immutable semantic view with questions such
as `uses_name_column_mapping`, `change_data_feed_enabled`, and
`type_widening_enabled`. Keep the view derived from the property mapping so it
cannot disagree with desired state. Do not introduce a generic backend-policy
interface while there is only one implementation.

## Follow-up review consolidation

The later reviews are folded here by outcome so their repeated context and
recommendations do not become parallel sources of truth.

### PR #337: policy containment

| Original finding | Consolidated disposition |
| --- | --- |
...
| Work item | Status |
| --- | --- |
| Semantic primary-key equality and matching | ✅ Implemented by #342. |
| Remove escaped key-signature projections | ✅ Implemented by #343. |
| Expose foreign-key column pairs as one operation | Open, low priority; one pair-oriented production consumer remains. |
| Harden public and domain constraint construction | ✅ Implemented by #344. |
| Redesign foreign-key admission authority | Deferred to findings 1 and 2; deliberately excluded from the contained value changes. |
