...
- [ ] Tighten `ForeignKeyConstraint.constraint_name` so a desired FK reaching the differ is statically named, removing the `assert foreign_key.constraint_name is not None` in `_diff_foreign_keys` AND the twin `if foreign_key.constraint_name is not None` guard on the drop path (same underlying looseness: the field is `str | None` but a `DesiredTable`'s FKs are always named by `with_generated_name`; the `None` state only exists transiently before `DesiredTable.__post_init__` names them). Ousterhoutian: define the error out of existence via a named-constraint type or construction-time naming, rather than narrowing `str | None → str` at the diff site. Superseded and expanded by cluster A of the [special-case audit](2026-07-03-special-case-audit.md), which found the same looseness also guards `_diff_primary_key` and the CREATE TABLE compiler.
