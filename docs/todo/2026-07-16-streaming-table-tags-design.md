# Tags scope on streaming tables and materialized views — design
...
Databricks rejects `ALTER TABLE` statements against streaming tables and
materialized views; those relation kinds require `ALTER STREAMING TABLE` and
`ALTER MATERIALIZED VIEW`. The tags scope (`DeltaTable(scope="tags")`) was
built assuming one ALTER dialect for all tables, and its motivating use case —
managing tags on tables owned elsewhere, e.g. by a streaming pipeline — is
exactly the case the assumption breaks on.
...
- `ALTER STREAMING TABLE` and `ALTER MATERIALIZED VIEW` support `SET TAGS`,
  `UNSET TAGS`, and `ALTER COLUMN ... SET TAGS` / `UNSET TAGS`.
- Neither supports constraint clauses; `SET TBLPROPERTIES` is not in the
...
2. **Kinds**: streaming tables and materialized views. Views, foreign
   tables, and unknown kinds keep failing closed at read.
3. **Policy**: only declarations whose managed aspects are tag aspects may
   target an ST/MV. `scope="full"` and `scope="metadata"` against one fail
   validation.
...
- The admit-gate changes from a set of relation types to a mapping onto
  ...
  `STREAMING_TABLE → STREAMING_TABLE`, `MATERIALIZED_VIEW → MATERIALIZED_VIEW`.
- Everything else still raises `UnsupportedRelationError` (surfaced as
  `ReadFailed`), including kinds Databricks adds in the future. The error
  message no longer lists streaming tables among the unmanageable kinds.
- Open fact to pin live before writing the gate: the `type` and `provider`
  values `DESCRIBE ... AS JSON` reports for an ST and an MV. Whether the
  existing `provider == "delta"` check extends to ST/MV is written against
  the pinned values, not guessed.
- The information_schema follow-ups (tags, constraints) run unchanged;
  STs/MVs simply return no constraints.
...
- New scope-gate peer alongside `UnmanagedAspectDrift` and
  ...
  Working name: `PipelineOwnedRelationTagsOnly`.
- Semantics: when `TableDrift.kind` is `STREAMING_TABLE` or
  `MATERIALIZED_VIEW` and the declaration's managed aspects are not a subset
  of the tag aspects, produce one failure naming the observed kind and
  pointing the user at `scope="tags"`.
...
- Existing behaviour composes for free: an absent ST/MV under tags scope
  still fails `MissingTableUnmanaged` (tags scope does not manage
  existence), and structural drift under tags scope still fails
...
- The hardcoded `ALTER TABLE {name}` prefix becomes a kind-dispatched
  prefix — `ALTER TABLE` / `ALTER STREAMING TABLE` /
  `ALTER MATERIALIZED VIEW` — applied mechanically to every ALTER-family
  statement via one prefix helper, not a per-action matrix.
- The compiler stays policy-free. It does not know that `AddColumn` on a
  materialized view is nonsense; validation made that unreachable. Handed
...
- Under the approved policy, only the four tag statements (table set/unset,
  column set/unset) ever carry the two new prefixes; every other ALTER
  statement keeps compiling against ordinary tables as `ALTER TABLE`.
- `CREATE TABLE` and `COMMENT ON` compilation is untouched: creates are
  always ordinary tables, and comment actions cannot reach an ST/MV past
  the gate.
...
- Reader: describe fixtures for an ST and an MV map to the right `kind`; a
  view still reads as failed.
- Validation: tags scope + ST/MV drift passes the gate; `full` and
  `metadata` fail it, including with zero drift; absent ST under tags scope
  still fails `MissingTableUnmanaged`; structural drift under tags scope
  still fails `UnmanagedAspectDrift`.
- Compiler: the four tag statements compile under each of the three
  prefixes; an end-to-end dry-run sync against an observed ST produces
...
1. `DESCRIBE ... AS JSON` `type`/`provider` values for a real ST and MV —
   the reader gate is written against these.
2. `ALTER STREAMING TABLE` / `ALTER MATERIALIZED VIEW` `SET TAGS`,
   `UNSET TAGS`, and `ALTER COLUMN ... SET TAGS` succeed.
3. Plain `ALTER TABLE ... SET TAGS` against an ST fails — pinning the
   premise of the feature; if Databricks ever accepts it, the pin flags the
   dispatch as obsolete.
4. information_schema tag reads return ST/MV tags.
5. Round-trip: a tags-scope sync against a live ST reconciles a tag set and
   a tag unset.

Provisioning uses `CREATE STREAMING TABLE` / `CREATE MATERIALIZED VIEW` on
the SQL warehouse (implicit pipeline spin-up; the Live run gets slower),
with teardown drops.

## Documentation

- `how-to-deploy-metadata-only.md`: tags scope works on STs/MVs; example.
...
- `reference-limitations.md`: narrow "streaming tables unsupported" to the
  precise truth — tags-only management.
- `explanation-safety-model.md`: pipeline-owned relations as tags-only
  territory.
...
- Managing comments, properties, constraints, schedules, row filters, or
  ownership on STs/MVs.
- Creating streaming tables or materialized views.
...
- **Adapter-only dispatch** (reader admits ST/MV, executor re-describes or
  caches kind; no domain/validation change): validation cannot see kind, so
  a full-scope declaration against an ST reaches execution and fails as a
  backend error — the safety model's job done by the backend. Blocking
  non-tag actions inside the adapter would mean the adapter deciding
  safety, which the architecture forbids.
...
- The `provider` value for STs/MVs in `DESCRIBE ... AS JSON` is unverified
  until the live pin lands; the reader gate is written after pinning.
- Live provisioning cost: streaming-table creation spins up a managed
  pipeline per Live run (accepted).
- If Databricks later accepts `ALTER TABLE` on STs/MVs, the dispatch
  becomes unnecessary but harmless; live pin 3 detects the shift.
