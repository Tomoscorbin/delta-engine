...
| Add a lint rule                   | `delta_engine.lint.rules`                                                  | One dataclass satisfying `LintRule`, registered in `ALL_RULES`; its fields are its config parameters, and config parsing, known settings, and defaults all derive from the registry.                                                        |
