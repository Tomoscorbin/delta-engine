...
`ColumnTypeChange`, `PartitioningChange`, `TargetTableMissing`, `TargetColumnMissing`, and `UnenforceablePrimaryKey` are actions that are never executed. The differ emits them to describe a condition it detected without judging whether that condition is allowed; deciding what is permitted is the validation layer's job, and each sentinel has a rule that rejects it with a clear message. `ColumnTypeChange` and `PartitioningChange` describe drift — a column whose type differs, or a changed partition spec. The three broken-target actions describe managed metadata that cannot land — a missing table, a missing column, or a primary key over live-nullable columns — and are emitted only when column structure is unmanaged. The SQL compiler raises `AssertionError` if any sentinel reaches compilation — encoding the invariant that validation always runs first.
