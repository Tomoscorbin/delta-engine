...
`ForeignKey(name=...)` is supplied. An unnamed declaration accepts any
structurally matching observed constraint; an explicit name is managed state.
Databricks has no direct constraint-rename clause, so changing a managed name
