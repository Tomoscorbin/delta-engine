...
| Reads (both backends) | Unity Catalog only — every existing-table read uses `DESCRIBE TABLE EXTENDED … AS JSON`, `DESCRIBE DETAIL` for supported table features, and `information_schema` for tags, primary and foreign keys, and inbound foreign keys; a `hive_metastore` or other non-UC table is not readable and surfaces as a read failure on both backends ([installation](installation.md)) |
