...
Before execution, delta-engine rejects explicit-name collisions among the
constraint additions in the current sync. It does not read every constraint in
the schema, so a collision with an unseen or externally managed constraint can
still fail at Databricks execution.
...
| Loosen nullability        |       ✓       | `NOT NULL` → nullable is applied to top-level columns; nested field changes are blocked as struct type changes                                                                                                            |
| Tighten nullability       |       ✗       | Top-level columns are blocked — backfill first, then tighten; nested field changes are blocked as struct type changes ([rules](reference-safe-change-rules.md))                                                            |
...
| Primary keys              |       ✓       | Declared at table level; definitions are managed, while an optional name is used only when adding the key ([primary keys](how-to-configure-table.md#primary-keys))                                                        |
| Foreign keys              |       ✓       | Must target the referenced table's primary key and order the sync; definitions are managed, while optional names are creation preferences ([foreign keys](how-to-configure-table.md#foreign-keys))                        |
