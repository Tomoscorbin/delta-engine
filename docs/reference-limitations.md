...
Databricks Runtime 16.2 or later, or any SQL warehouse. Observing primary- and
foreign-key constraints from that output needs Databricks Runtime 17.3 or later,
or any SQL warehouse; on an older Spark cluster the read succeeds but reports no
constraints. As with every other runtime feature, delta-engine documents this
floor rather than preflighting it — an unsupported runtime surfaces as a read
failure.
