...
| Rename column             |       ✓       | Declare `renamed_from` on the new column; requires `delta.columnMapping.mode='name'`. An undeclared rename (drop + same-typed add) is blocked ([renaming a column](how-to-configure-table.md#renaming-a-column)) |
| Rename partition column   |       ✗       | Blocked: renaming a partition column reads as an in-place partitioning change ([rules](reference-safe-change-rules.md))                                                                                          |
