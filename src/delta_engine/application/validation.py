...
class PrimaryKeyReferencedByForeignKeys:
    ...
    *this* table that this same sync also drops is exempt — DROP_FOREIGN_KEY
    ...
    def evaluate(self, drift: TableDrift) -> tuple[ValidationFailure, ...]:
        ...
        dropped_here = {
            change.name for change in drift.actions if isinstance(change, DropForeignKey)
        }
        failures: list[ValidationFailure] = []
        for change in drift.actions:
            ...
            blockers = tuple(
                ...
                if not (
                    ...
                    and reference.name in dropped_here
                )
            )
            if blockers:
                referenced_by = ", ".join(
                    f"{ref.name} on {ref.referencing_table}" for ref in blockers
                )
                failures.append(
                    ValidationFailure(
                        rule_name=self.name,
