...
def _build_dependency_graph(
    ...
    Self-referential foreign keys cannot occur here: ``DesiredTable`` rejects
    them, so no table is ever its own dependency.
    """
    ...
    for table in tables:
        for foreign_key in table.foreign_keys:
            if foreign_key.references in registered_names:
                graph[table.qualified_name].add(foreign_key.references)
    return graph
...
def _strongly_connected_components(
    ...
    always precedes its dependents'. Neighbours are visited in name order and
    nodes in graph (registry) insertion order, making the result deterministic
    regardless of set-iteration order or hash seed.

    A component of more than one node is a true dependency cycle. (Self-loops
    never occur — see :func:`_build_dependency_graph` — so a single node is
    never cyclic.)
    """
    index_counter = 0
