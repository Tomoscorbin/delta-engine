...
class ForeignKey:
    ...
    ``name`` optionally requests the physical constraint name at creation.
    When omitted, Databricks chooses the name. Existing constraints match by
    definition regardless of their catalog name.
...
def _lower_declaration(declaration: _NormalizedDeclaration) -> DesiredTable:
    ...
    primary_key_constraint = (
        DesiredPrimaryKey(
            ...
            desired_name=declaration.primary_key_name,
        )
...
class DeltaTable:
    ...
    def __init__(
        ...
        Args:
            ...
            foreign_keys: Foreign key relationships declared on this table.
                Each may request a physical constraint ``name`` at creation;
                Databricks names constraints whose declaration omits it.
    ...
    def primary_key_name(self) -> str | None:
        """Explicit primary-key creation name, or ``None`` when omitted."""
        primary_key = self._desired_table.primary_key
        if primary_key is None or primary_key.desired_name is None:
            return None
        return str(primary_key.desired_name)
