...
The information_schema queries are Unity Catalog only; the reader owns the
policy for environments where information_schema does not exist. The
structured DESCRIBE query is the SQL warehouse reader's primary table read.
"""
...
def describe_json_query(qualified_name: QualifiedName) -> str:
    """Render the warehouse reader's structured primary metadata query."""
