...
def _(action: CreateTable, target: _SqlTarget) -> str:
...
@_compile_action.register
def _(action: AddColumn, target: _SqlTarget) -> str:
    """
    ...
    return f"{target.alter_table} ADD COLUMN {name} {dtype}{comment}"
...
@_compile_action.register
def _(action: RenameColumn, target: _SqlTarget) -> str:
    """Compile ALTER TABLE ... RENAME COLUMN."""
    old = backtick(action.old_name)
    new = backtick(action.new_name)
    return f"{target.alter_table} RENAME COLUMN {old} TO {new}"


@_compile_action.register
def _(action: SetProperty, target: _SqlTarget) -> str:
    pair = f"{quote_literal(action.name)}={quote_literal(action.desired_value)}"
    return f"{target.alter_table} SET TBLPROPERTIES ({pair})"


@_compile_action.register
def _(action: UnsetProperty, target: _SqlTarget) -> str:
    key = quote_literal(action.name)
    return f"{target.alter_table} UNSET TBLPROPERTIES IF EXISTS ({key})"
...
@_compile_action.register
def _(action: SetColumnComment, target: _SqlTarget) -> str:
    ...
    return f"{target.alter_table} ALTER COLUMN {column_name} COMMENT {comment}"


@_compile_action.register
def _(action: SetTableComment, target: _SqlTarget) -> str:
    comment = quote_literal(action.desired_comment)
    return f"COMMENT ON TABLE {target.name} IS {comment}"


@_compile_action.register
def _(action: SetColumnNullability, target: _SqlTarget) -> str:
    column_name = backtick(action.column_name)
    sign = "DROP" if action.desired_nullable else "SET"
    return f"{target.alter_table} ALTER COLUMN {column_name} {sign} NOT NULL"


@_compile_action.register
def _(action: AlterClustering, target: _SqlTarget) -> str:
    """Compile ALTER TABLE ... CLUSTER BY (...) or CLUSTER BY NONE."""
    if not action.desired_clustering:
        return f"{target.alter_table} CLUSTER BY NONE"
    columns = ", ".join(backtick(column) for column in action.desired_clustering)
    return f"{target.alter_table} CLUSTER BY ({columns})"


@_compile_action.register
def _(action: AlterColumnType, target: _SqlTarget) -> str:
    """Compile ALTER TABLE ... ALTER COLUMN ... TYPE for a validated type widening."""
    column_name = backtick(action.column_name)
    sql_type = render_data_type(action.desired_type)
    return f"{target.alter_table} ALTER COLUMN {column_name} TYPE {sql_type}"
...
@_compile_action.register
def _(action: SetPrimaryKey, target: _SqlTarget) -> str:
    ...
    return f"{target.alter_table} ADD CONSTRAINT {constraint} PRIMARY KEY ({column_clause})"


@_compile_action.register
def _(action: DropForeignKey, target: _SqlTarget) -> str:
    ...
    return f"{target.alter_table} DROP CONSTRAINT IF EXISTS {constraint}"


@_compile_action.register
def _(action: SetForeignKey, target: _SqlTarget) -> str:
