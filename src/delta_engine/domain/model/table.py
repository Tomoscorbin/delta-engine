...
class DesiredTable(TableSnapshot):
    ...
    is generated by the API layer when a ``DeltaTable`` is lowered, and for
    observed constraints the name is read from the catalog. The differ and
    compiler read the name off the constraint rather than deriving it themselves.
    """
