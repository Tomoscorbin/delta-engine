...
class ReaderNone(CatalogReader):
    def fetch_state(self, qualified_name: QualifiedName):
        return None
...
class ExecFail(PlanExecutor):
    ...
    def execute(self, plan: ActionPlan) -> ExecutionOutcome:
        ...
        return ExecutionOutcome(
            success=False, messages=(self._msg,), executed_count=max(0, len(plan) - 1)
        )
...
def test_plan_then_execute_returns_noop_without_calling_executor_when_no_actions(monkeypatch):
    ...
    res = plan_then_execute(
        desired_table=desired("a"),
        reader=ReaderWith(observed("A")),
        executor=ExecFail("should not run"),
    )
